...
#cs ----------------------------------------------------------------------------
	...
	changelog:
	0.0.0.1 initial relase
	0.0.0.2 fixed bug :array dimension exceeded after removing all elements from TV and adding new folder
...
Func _MenuAdd_EV()
	...
	If _GUICtrlTreeView_GetCount($TreeView_MM_Edit) = 0 Then
		...
		;check if aux array size went under 2 due to removing elements
		;if so, redim array

		If UBound($AUX_TV_Array, $UBOUND_ROWS) < 3 Then ReDim $AUX_TV_Array[3][2]
